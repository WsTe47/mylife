...
**MyLife 不是心理支持工具。** 若你正经历严重心理痛苦，请寻求专业帮助，不要依赖本工具。

**但有一件事要说清楚：** 有很多心事、烦心事，人是不便于和真人说的 ——
**这正是这个工具存在的理由，不是缺陷。** MyLife 不会因为你有心事就劝你"去找人聊聊"。
它做的是**辅助分析**：把混乱的心事理成一条你能自己看清楚的线。

只有当痛苦反复出现并影响生活、或出现危机信号时，它才会暂停规划并提示专业支持 ——
而且会说明**不必是认识的人**。分层设计见
[敏感场景与心理边界](docs/design/sensitive-scenarios.md)。
...
- **词法极性引擎（`mylife_counter_evidence`）在真实语料上常常一条都找不到。**
  它只匹配字面反义词，抓不到价值观层面的对立（实测：23 条证据、0 findings）。
  已被**证伪链**（`mylife_defeaters`）取代为主路径，它退为零成本的快速预筛。
- **证伪链需要 LLM 调用**（每次约 10 秒）。没有凭据时会降级到词法引擎，
  并在报告里显式告警"结论缺少反方检验"。
...
- [x] 四层信息模型 + 溯源链（23/23 精确到行，审计通过）
- [x] 时效引擎：逐字段 TTL + 依赖图 + **`unset`（从未填写）状态**
      —— "声明了依赖但从没填过"与"值过期"分开报，措辞与出路都不同
- [x] **证伪链**（反证主路径）：主判断 → 生成证伪项（internal / gap）→ 四态判定
      `A` 削弱 / `B` 被事实否定 / `R` 本人已想过并排除 / **`C` 空白**
- [x] **已知事实作为证据源** —— 否则用户补了数据，证伪链仍报"空白"（闭环会断）
...
| [敏感场景与心理边界](docs/design/sensitive-scenarios.md) | 三层模型；为什么不能劝用户"去找人聊聊" |
| [证伪链](docs/design/defeater-chain.md) | 反证引擎为何重做、实测对比、必须守住的措辞约束 |
| [后续规划视角](docs/design/next-planning-angles.md) | 五个规划视角、我的建议、待决定的问题 |
